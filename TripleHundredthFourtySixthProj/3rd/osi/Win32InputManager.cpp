...
BOOL CALLBACK Win32InputManager::_DIEnumDevCallback(LPCDIDEVICEINSTANCE lpddi, LPVOID pvRef)
{
		...
		GET_DIDEVICE_TYPE(lpddi->dwDevType) == DI8DEVTYPE_FLIGHT)
	{
		...
		jsInfo.vendor = ConvertWideToNarrow(lpddi->tszInstanceName);
...
static std::string ConvertWideToNarrow(const WCHAR* wideStr) {
	if (!wideStr) return ""; // Handle null pointer case

	int bufferSize = WideCharToMultiByte(CP_UTF8, 0, wideStr, -1, nullptr, 0, nullptr, nullptr);
	if (bufferSize == 0) return ""; // Check for conversion failure
